...
function toggleSubAjax(id, elemento) {
    ...
    fetch(`mete.php?azione=toggle_sub&id=${id}&stato=${isChecked}&ajax=1`)
        ...
        .catch(error => {
            ...
            alert("Errore di connessione. Le modifiche potrebbero non essere salvate.");
        });
}
...
function toggleMainAjax(id, elemento) {
    ...
    const testo = card.querySelector('.meta-text') || card.querySelector('div[style*="margin-bottom"]');
...
function eliminaSubAjax(id, elemento) {
    ...
    fetch(`mete.php?azione=elimina_sottometa&id=${id}&ajax=1`)
    .then(response => {
        if (!response.ok) console.error("Errore durante l'eliminazione sul server");
    });
}

// --- LOGICA TIMER CON PERSISTENZA ---

let startTime, tInterval, running = false, savedTime = 0;
    ...
    if (ore === 0 && minuti === 0) {
        ...
        if (btnCancel) {
            ...
            const ripristinaBottone = () => {
                ...
                document.getElementById('confirmOk').removeEventListener('click', ripristinaBottone);
            };
            document.getElementById('confirmOk').addEventListener('click', ripristinaBottone);
        }
        return;
    }

    window.chiediConferma("Salva Tempo", `Registrare <strong>${ore}h</strong> e <strong>${minuti}m</strong>?`, function() {
        const urlParams = new URLSearchParams(window.location.search);
        const m = urlParams.get('m') || (new Date().getMonth() + 1);
        const a = urlParams.get('a') || new Date().getFullYear();
        
        fetch(`servizio.php?azione=salva_timer&ore=${ore}&minuti=${minuti}&m=${m}&a=${a}&ajax=1`)
            .then(res => res.json())
            .then(data => {
...
document.addEventListener('submit', function(e) {
    if (e.target.closest('form') && e.target.querySelector('button[name="salva"]')) {
        e.preventDefault();
        const form = e.target;
...
function aggiornaUI(totali) {
    // Aggiorna H1
    document.querySelector('.totale-box h1').innerText = `${totali.h_m}h ${totali.m_m}m`;
...
function aggiornaGraficaObiettivi(oreMese, oreAnno) {
    ...
    const labelMese = document.querySelector('.progress-bar-mensile')?.parentElement?.previousElementSibling;
    const labelAnno = document.querySelector('.progress-bar-anno')?.parentElement?.previousElementSibling;

    if (labelMese) {
        const targetMese = parseInt(labelMese.innerText.split('/')[1]);
        labelMese.querySelectorAll('span')[1].innerText = `${oreMese} / ${targetMese}h`;
        const percMese = Math.min(100, Math.round((oreMese / targetMese) * 100));
        document.querySelector('.progress-bar-mensile').style.width = percMese + '%';
    }

    if (labelAnno) {
        const targetAnno = parseInt(labelAnno.innerText.split('/')[1]);
        labelAnno.querySelectorAll('span')[1].innerText = `${oreAnno} / ${targetAnno}h`;
...
function aggiornaBarraProgressiva() {
    ...
    meteCard.forEach(card => {
        ...
        // 1. Controlliamo se la meta principale è spuntata
        const mainCheck = card.querySelector('.btn-check');
        const isMainChecked = mainCheck && mainCheck.classList.contains('checked');
        
        // 2. Controlliamo le sotto-mete
        const sottoMete = card.querySelectorAll('.sub-meta-item');
        
        if (sottoMete.length > 0) {
            // Se esistono sotto-mete: Meta principale = 33.3%, Sotto-mete = 66.6%
            if (isMainChecked) goalVal += (1 / 3);
            
            // Contiamo quante sotto-mete hanno la classe .completed o .checked
            const completate = card.querySelectorAll('.btn-check-small.checked').length;
            goalVal += (completate / sottoMete.length) * (2 / 3);
        } else {
            // Se non ci sono sotto-mete, il check principale vale il 100%
    ...
    if (progressText) {
        ...
        // Log di debug: se vedi questo in console, l'ID nel PHP è sbagliato
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.delete-link').forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            const riga = this.closest('.meta-item') || this.closest('.meta-wrapper') || this.closest('.meta-card'); 

            window.chiediConferma("Elimina", "Sei sicuro di eliminare la meta?", () => {
                // Fetch con parametro ajax=1
                fetch(`mete.php?azione=elimina&id=${id}&ajax=1`)
                .then(() => {
                    if(riga) {
                        riga.style.opacity = '0';
                        ...
                        setTimeout(() => {
                            riga.remove();
                            if (typeof aggiornaBarraProgressiva === 'function') aggiornaBarraProgressiva();
                        }, 300);
                    }
                });
            });
